...
def make_plot2(data_file, output, show=False):
    ...
    win = signal.hann(300)
    start = 200
    filtered_temp = np.array(signal.convolve(temp, win, mode='same') / np.sum(win))[start:]
    filtered_temp_time = np.array(temp_time[start:])
...
def make_plot(data_file, output, show=False):
    ...
    df.to_excel("F:/filtered_temp_data.xlsx")
   ...
   # ax1.plot(core_temp_seires2.keys(), core_temp_seires2.values, 'g', label="Estimated from Pulse Monitor")
    ...
    # Heart Rate Plot
    ax2.plot(hr_time, HR, 'k', alpha=raw_alpha, label=" HR")
...
def main():
    ...
    #show=args.show
   # make_plot('F:\jon 2-17-17.xls', 'F:', true)
    if args.debug:
